...
VALUES (NEW."ID_OBS",NEW.geom,NEW.util_geom,NEW."Type_OBS");

INSERT INTO public."Evenement" ("ID_EVEN","ID_Employ","ID_Proprio","ID_Proj","Verif_Err","ID_OBS_L","Heure","Date","Utilisateur","Comment","Type_Obs","ID_Station","ID_Licence","Proprio_donnee","Prive")
VALUES (NEW."ID_EVEN",NEW."ID_Employ",NEW."ID_Proprio",NEW."ID_Proj",NEW."Verif_Err",NEW."ID_OBS_L",NEW."Heure",NEW."Date",NEW.util_even,NEW."Comment",NEW."Type_Obs",NEW."ID_Station",NEW."ID_Licence",NEW."Proprio_donnee",NEW."Prive");
...
CREATE OR REPLACE VIEW public.form_deterce_v
 ...
 SELECT a."ID_OBS",
   ...
   FROM "Obs_Ligne" a,
    "Evenement" b,
    form_deterce c
  WHERE c.utilisateur::text ~~ concat(USER, '%') OR replace(USER::text, "substring"(USER::text, "position"(USER::text, '_'::text)), ''::text) = c.utilisateur::text;
