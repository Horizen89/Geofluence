-- sp veget mhh caractcourseau
-- depends: 20250609_01_R9Az9-caractcourseau-erosion-2025

-- Ajout d'une colonne dans FormSect_SP_Veget

DROP VIEW "FormSect_SP_Veget_V";

ALTER TABLE IF EXISTS public."FormSect_SP_Veget"
    ADD COLUMN "Rive" text COLLATE pg_catalog."default";

CREATE OR REPLACE VIEW public."FormSect_SP_Veget_V"
 AS
 SELECT *
   FROM "FormSect_SP_Veget"
  WHERE "FormSect_SP_Veget"."Utilisateur" ~~ concat(USER, '%') OR replace(USER::text, "substring"(USER::text, "position"(USER::text, '_'::text)), ''::text) = "FormSect_SP_Veget"."Utilisateur";

ALTER TABLE public."FormSect_SP_Veget_V"
    OWNER TO geof_maitre;

GRANT INSERT, SELECT ON TABLE public."FormSect_SP_Veget_V" TO geof_invite;
GRANT ALL ON TABLE public."FormSect_SP_Veget_V" TO geof_maitre;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public."FormSect_SP_Veget_V" TO geof_utilisateur;
...
CREATE OR REPLACE VIEW public."Form_MH_V"
 AS
 SELECT *
   FROM "Form_MHH"
  WHERE "Form_MHH"."Utilisateur" ~~ concat(USER, '%') OR replace(USER::text, "substring"(USER::text, "position"(USER::text, '_'::text)), ''::text) = "Form_MHH"."Utilisateur";

ALTER TABLE public."Form_MH_V"
    OWNER TO geof_maitre;

GRANT INSERT, SELECT ON TABLE public."Form_MH_V" TO geof_invite;
GRANT ALL ON TABLE public."Form_MH_V" TO geof_maitre;
GRANT INSERT, SELECT, UPDATE, DELETE ON TABLE public."Form_MH_V" TO geof_utilisateur;

-- nouvelle table pour caracterisation de cours d'eau

-- CaractCoursEau_Connect
